...
class Salesorder extends Admin_Controller {
    ...
    function saveitemso(){
        ...
        $diskon_standar = $this->input->post('diskon_standar_persen') * $this->input->post('qty_order') * $this->input->post('harga_normal') / 100;
        ...
        $diskon = $diskon_standar + $diskon_promo_rp + ($diskon_promo_persen * $harga_normal * $qtyorder / 100);
    ...
    function saveheaderso(){
        ...
        foreach($data_tmp as $key=>$val){
            ...
            if ($val->qty_booked < $val->qty_order) {
              ...
              //$keys = array_shift($dataitem);
              //$dataitem_so = array_push($dataitem_pending, $dataitem);
              $this->db->insert('trans_so_pending_detail',$dataitem_pending);
              //$this->db->where($dataitem_pending);
              //$this->db->update('trans_so_pending_detail',$dataitem);
            }
            else {
              //$qty_supply_pending = '0';
              //$keys = array_shift($dataitem);
