...
class Invoice extends Admin_Controller {
    ...
    */
    ...
    function print_custom_lawas($noinv){
      ...
      $mpdf->SetImportUse();
        ...
        <div style="display: inline-block; position:relative;width:100%;display: none;">
          <div style="width:25%">
        	...
        	<table width="100%" border="0" id="header-tabel" style="font-size:10pt !important;">
  	      	<tr>
          ...
          <table width="100%" border="0" id="header-tabel" style="font-size:10.5pt !important;">
            <tr>
                <td width="5%">NO. REFF</td>

                <td colspan="3" width="60%">: '.@$inv_data->no_invoice.'</td>
            ...
            <tr>
                ...
                <td colspan="3">:
                    '.$telat->days.' HARI &nbsp;&nbsp; JATUH TEMPO : '.date('d/m/Y',strtotime(@$inv_data->tgljatuhtempo)).'
                </td>
        ...
        <hr>
        <table width="100%" border="0" style="font-size:9.5pt">

            <tr>
                <td colspan="3">
                    <i>TERBILANG : <strong>'.ucwords(ynz_terbilang_format(@$inv_data->hargajualtotal)).'</strong></i>
                </td>
                <td width="19%"></td>
                <td width="1%"></td>
            ...
            <tr>
                ...
                <td width="1%">:</td>
                <td width="15%" style="text-align: right;"><strong>'.formatnomor(@$inv_data->hargajualafterdis).'</strong></td>
                <!--<td width="10%"></td>-->
            </tr>
            <tr>
                <td colspan="3"></td>
                ...
                <td width="1%">:</td>
                <td width="15%" style="text-align: right;"> <strong>'.formatnomor(ceil($dis_toko_rp)).'</strong></td>
                <!--<td width="10%"></td>-->
            </tr>
            <tr>
                <td colspan="3">
                ...
                <td width="1%">:</td>
                <td width="15%" style="text-align: right;"><strong>'.formatnomor(ceil($dis_cash_rp)).'</strong></td>
                <!--<td width="10%"></td>-->
            </tr>
            <tr>
                <td colspan="3">
                    ...
                    <td width="1%">:</td>
                    <td width="15%" style="text-align: right;"><strong>'.formatnomor(@$inv_data->hargajualtotal).'</strong></td>
                </td>
            </tr>
            <tr>
                <td colspan="6">
    ...
    function print_custom_new($noinv){
      $mpdf=new mPDF('utf-8', array(210,145), 10 ,'Arial', 5, 5, 16, 16, 1, 4, 'P');
      $mpdf->SetImportUse();
        $session = $this->session->userdata('app_session');
        $inv_data = $this->Invoice_model->find_by(array('no_invoice' => $noinv));
        $customer = $this->Invoice_model->cek_data(array('id_customer'=>$inv_data->id_customer),'customer');
        $prov = $this->Invoice_model->cek_data(array('id_prov'=>$customer->provinsi),'provinsi');

        $detail = $this->Detailinvoice_model->find_all_by(array('no_invoice' => $noinv));

        $do_data = $this->Invoice_model->cek_data(array('no_do'=>$detail[0]->no_do),'trans_do_header');
        ...
        $header = '
          ...
          <table border="0" id="header-tabel" style="font-size:9pt !important;border-spacing:-1px;margin: 0 15% 0 15%; min-width:70%">
            <tr>
                <td width="28%">NO. Surat Jalan</td>

                <td width="25%">: '.@$detail[0]->no_do.'</td>
                <td width="25%">NO. Invoice</td>
                <td width="1%">: '.@$inv_data->no_invoice.'</td>
            </tr>
            <tr>
                <td>Sales</td>

                <td>: '. @$inv_data->nm_salesman.'</td>
                <td>Kirim Lewat Ekspedisi</td>
                <td width="1%">: '.@$do_data->nm_supir.'</td>
            ...
            <tr>
                ...
                <td>: '.@$inv_data->tgljatuhtempo.'</td>
                <td>
                    Keterangan
                </td>
                <td width="1%">: '.@$inv_data->keterangan.'</td>
            </tr>
          </table>';
        ...
        <table border="0" style="border-spacing:-1px;width:100%">
          <tr>
            <td colspan="2" style="text-align:left;border:1px solid #000;border-spacing:1px !important">
              <i>Terbilang : <strong>'.ucwords(ynz_terbilang_format(@$inv_data->hargajualtotal)).'</strong></i>
            </td>
            ...
            <th rowspan="3" style="text-align:left;vertical-align:top;padding:0 0 0 1%">
              :'.formatnomor(@$inv_data->hargajualafterdis).'<br>
              :'.@$inv_data->diskon_toko_persen.' %<br>
              :'.@$inv_data->diskon_cash_persen.' %<br>
              :'.formatnomor(@$inv_data->hargajualtotal).'<br>
        ...
        <table>
            <tr>
              ...
              CATATAN :<br>
              1. Sudah termasuk PPN 10%<br>
              2. Faktur ini dianggap lunas, jika Cek / Bilyet Giro telah bisa dicairkan<br>
              3. Penagihan dianggap sah hanya jika dengan FAKTUR ASLI & SURAT JALAN ASLI<br>
              4. Barang yang belum lunas masih merupakan titipan PT. Importa Jaya Abadi<br>
              5. Pembayaran dengan BG/Cek harap diisi nama “ PT. IMPORTA JAYA ABADI ”
              </td>
            </tr>
        </table>
        ');
