...
class Barang_model extends BF_Model
{
    ...
    public function get_kode_barang($param,$id_group,$id_supplier) {
      $query = $this->db->query("SELECT MAX(id_barang) as max_id FROM barang_master WHERE id_group = '".$id_group."' and id_supplier = '".$id_supplier."'");
      $row = $query->row_array();
      $max_id = $row['max_id'];
      $max_id1 =(int) substr($max_id,10,3);
    ...
    public function get_data($kunci,$tabel) {
        $this->db->where($kunci);
        //$this->db->join('barang_master','barang_koli.id_barang=barang_master.id_barang','left');
