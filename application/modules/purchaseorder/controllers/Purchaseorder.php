...
class Purchaseorder extends Admin_Controller
{
    ...
    public function proses()
    {
        ...
        $this->template->set('total_pr', count($getparam));
        ...
        $this->template->title('Input Purchase Order999');
    ...
    public function hapus_header_po()
    {
    ...
    public function tezzz()
    {
        ...
        for ($i = 0; $i < count($detail['nopr_topo']); ++$i) {
            $group_pr[$_POST['nopr_topo'][$i]] = $group_pr[$_POST['nopr_topo'][$i]] + $this->input->post('qty_pr')[$i];
            $group[$_POST['nopr_topo'][$i]] = $group[$_POST['nopr_topo'][$i]] + $_POST['qty_supply'][$i];
        }

        foreach ($group as $type => $total) {
            if ($group["$type"] == $group_pr["$type"]) {
                $this->db->where(array('no_pr' => $type))
                    ->update('trans_pr_header', array('proses_po' => '1'));
            } else {
                $this->db->where(array('no_pr' => $type))
                    ->update('trans_pr_header', array('proses_po' => 'Pending'));
            }
        }
    }

    public function saveheaderponeww()
    {
        ...
        for ($i = 0; $i < count($detail['nopr_topo']); ++$i) {
            ...
            $dataitem_po = array(
                ...
                'qty_po' => $_POST['qty_supply'][$i], //$getitempr->qty_pr
        ...
        $group = array();
        $group_pr = array();
        for ($iz = 0; $iz < count($detail['nopr_topo']); ++$iz) {
            $group_pr[$_POST['nopr_topo'][$iz]] = $group_pr[$_POST['nopr_topo'][$iz]] + $this->input->post('qty_pr')[$iz];
            $group[$_POST['nopr_topo'][$iz]] = $group[$_POST['nopr_topo'][$iz]] + $_POST['qty_supply'][$iz];
        }
    ...
    public function print_request($nopo)
    {
        $mpdf = new mPDF('', '', '', '', '', '', '', '', '', '');
        $mpdf->SetImportUse();
    ...
    public function print_request_cabang($nopo)
    {
        ...
        $show = $this->template->load_view('print_request_cabang', $data);
