            ...
            <tbody id="isi_so">
              ...
              if ($detail) {
                foreach ($detail as $key => $value) {
                  ...
                  $harga_net = ($db->harga*((100-$db->diskon_standar_persen)/100))*((100-$db->diskon_promo_persen)/100)-($db->diskon_promo_rp);
                  if ($value['tipe_diskon_so'] == "rupiah_kurang") {
                    $harga_net = $harga_net-$value['diskon_so'];
                  }elseif ($value['tipe_diskon_so'] == "rupiah_tambah") {
                    $harga_net = $harga_net+$value['diskon_so'];
                  }else {
                    $harga_net = $harga_net-($harga_net*$value['diskon_so']/100);
                  }
                  ?>
                  <tr>
                    <td>
                    ...
                    <td>
                      <select onchange="setitembarang(this.value,this.id)" id="item_brg_so<?=$i?>" name="item_brg_so" class="form-control input-xs form_item_so select2" tabindex="-1">
                        <option value=""></option>
                        <?php
                    ...
                    <td>
                      <span id="item_brg_so<?=$i?>_qty">
                        <input type="text" class="form-control input-sm" id="qty_order_<?=$db->id_barang?>" onkeyup="qty_order_keyup('<?=$db->id_barang?>',this.value,'item_brg_so<?=$i?>');this.value = this.value.match(/^[0-9]+$/)" name="qty_order[]" style="width:100% !important;" value="<?=$value['qty_booked']?>">



                        <input type="hidden" class="form-control input-sm" id="id_barang_<?=$db->id_barang?>" name="id_barang[]" value="<?=$db->id_barang?>">
                        <input type="hidden" class="form-control input-sm" id="harga_<?=$db->id_barang?>" name="harga[]" value="<?=$db->harga?>">
                        <input type="hidden" class="form-control input-sm" id="harga_nett_<?=$db->id_barang?>" name="harga_nett[]" value="<?=$harga_net?>">
                        <input type="hidden" class="form-control input-sm" id="diskon_standar_persen_<?=$db->id_barang?>" name="diskon_standar_persen[]" value="<?=$db->diskon_standar_persen?>">
                        <input type="hidden" class="form-control input-sm" id="diskon_promo_persen_<?=$db->id_barang?>" name="diskon_promo_persen[]" value="<?=$db->diskon_promo_persen?>">
                        <input type="hidden" class="form-control input-sm" id="qty_stock_<?=$db->id_barang?>" name="qty_stock[]" value="<?=$db->qty_stock?>">
                        <input type="hidden" class="form-control input-sm" id="qty_avl_<?=$db->id_barang?>" name="qty_avl[]" value="<?=$db->qty_avl?>">
                        <input type="hidden" class="form-control input-sm" id="landed_cost_<?=$db->id_barang?>" name="landed_cost[]" value="<?=$db->landed_cost?>">
                    ...
                    <td>
                      <span id="item_brg_so<?=$i?>_diskon">
                        ...
                        <div class="input-group">
                          ...
                          <input type="text" class="form-control form_item_so input-sm" aria-label="" name="disso[]" id="disso_<?=$db->id_barang?>" class="input-sm" value="<?=$diskon_so?>" onkeyup="get_disso('item_brg_so<?=$i?>',this.value,'<?=$db->id_barang?>','<?=$i-1?>')">
                        </div><!-- /input-group -->
                        <div class="radio_disso_rp_<?=$db->id_barang?>" style="display:<?=$style?>">
                          <div class="radio-inline">
                            <label>
                              <input type="radio" value="tambah" id="<?=$i.'tambah'?>" name="radio_disso_rp[<?=$i-1?>]" <?=$rt?> onclick="get_disso('item_brg_so<?=$i?>',document.getElementById('disso_<?=$db->id_barang?>').value,'<?=$db->id_barang?>','<?=$i-1?>')">(+)
                            </label>
                          </div>
                          <div class="radio-inline">
                            <label>
                              <input type="radio" value="kurang" id="<?=$i.'kurang'?>" name="radio_disso_rp[<?=$i-1?>]" <?=$rk?> onclick="get_disso('item_brg_so<?=$i?>',document.getElementById('disso_<?=$db->id_barang?>').value,'<?=$db->id_barang?>','<?=$i-1?>')">(-)
                            </label>
                          </div>
                        </div>
                      </span>
                    ...
                    <td>
                      <span id="item_brg_so<?=$i?>_total"><?=formatnomor($value['subtotal'])?></span>
                    </td>
                  </tr>
                  <?php
                }
...
<script type="text/javascript">
  ...
  var Table = $('#listadjust').dataTable({
    ...
    $(document).ready(function() {
        ...
        $("#item_brg_so,#idcustomer,#idsalesman,#pic,#item_brg_so_bonus,.select2").select2({
            placeholder: "Pilih",
            allowClear: true
        });
