...
<div class="box">
  <div class="box-header text-left"><b>Pilih Status Konfirmasi : </b>
    <select id="type_return" name="type_return" class="form-control input-sm" style="width: 25%;" tabindex="-1" required onchange="gettype(this.id)">
        <option value=""></option>
        <?php
          ...
          foreach($type as $kso=>$vso){
            $selected = '';
            ?>
            <option value="<?php echo $vso->konfirm_do; ?>" <?php echo $selected?>>
          <?php echo $vso->konfirm_do ?>
        </option>
        <?php } ?>
    </select>
  </div>
