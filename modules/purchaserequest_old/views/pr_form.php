            ...
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    if($this->uri->segment(3) != ""){
                        ...
                        $caba = $this->Purchaserequest_model->cek_data(array('kdcab'=>$this->uri->segment(3)),'cabang');
                    }
                    ?>
                    <div class="col-sm-6">
                        <div class="form-group">
                          ...
                          <div class="col-sm-8">
                              ...
                              <select id="idcabang" name="idcabang" class="form-control input-sm" style="width: 100%;" tabindex="-1" required onchange="getcabang()">
                              <option value=""></option>
                              <?php
                              foreach(@$cabang as $cabb=>$vcabb){
                                  $selected ='';
                                  if($this->uri->segment(3) == $vcabb->kdcab){
                                      $selected='selected="selected"';
                                  }
                              ?>
                              <option value="<?php echo $vcabb->kdcab; ?>" <?php echo set_select('namacabang', $vcabb->kdcab, isset($data->namacabang) && $data->kdcab == $vcabb->kdcab) ?> <?php echo $selected?>>
                                  <?php echo $vcabb->kdcab.' , '.$vcabb->namacabang ?>
                              </option>
                              <?php } ?>
                              </select>
                        ...
                        if($this->uri->segment(4) != ""){
                            $supp = $this->Purchaserequest_model->cek_data(array('id_supplier'=>$this->uri->segment(4)),'supplier');
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                ...
                                foreach(@$supplier as $kc=>$vc){
                                    $selected ='';
                                    if($this->uri->segment(4) == $vc->id_supplier){
                                        $selected='selected="selected"';
                                    }
                                ?>
                    ...
                    <div class="col-sm-6">
                        ...
                        <!--
                        <div class="form-group ">
                            <?php //$realdeliverypr=date('Y-m-d')?>
                            <label for="realdeliverypr" class="col-sm-4 control-label">Real Delivery <font size="4" color="red"><B>*</B></font></label>
                            <div class="col-sm-8">
                                <div class="input-group">
                                    <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                                    <input type="text" name="realdeliverypr" id="realdeliverypr" class="form-control input-sm datepicker" value="<?php //echo $realdeliverypr?>">
                                    <input type="hidden" name="totalpr" id="totalpr" class="form-control input-sm">
                                </div>
                            </div>
                        </div>
...
<div class="box box-default ">
    <div class="box-body">
        ...
        if($this->uri->segment(5) != ""){
            $brg = $this->Purchaserequest_model->cek_data(array('id_barang'=>$this->uri->segment(5)),'barang_master');
        }
        ?>
                ...
                <td colspan="3" width="30%">
                    <select onchange="setitembarang()" id="item_brg_pr" name="item_brg_pr" class="form-control input-xs" style="width: 100%;" tabindex="-1" required>
                            ...
                            foreach(@$itembarang as $k=>$v){
                                $selected ='';
                                if($this->uri->segment(5) == $v->id_barang){
                                    $selected='selected="selected"';
                                }
                            ?>
        ...
        <?php
            $colly = $this->Purchaserequest_model->get_data(array('id_barang' => $this->uri->segment(5)),'barang_koli');
            $mastah = $this->Purchaserequest_model->get_data(array('id_barang' => $this->uri->segment(5)),'barang_master');
        ?>
        <table class="table table-bordered" width="100%">
            <tr>
        ...
        <table id="prdetailitem" class="table table-bordered table-striped" width="100%">
            ...
            <tbody>
               ...
               <tr>
                   ...
                   <td class="text-right"><?php echo formatnomor($vp->harga_satuan)?></td>
                   <td class="text-right"><?php echo formatnomor($vp->sub_total_pr)?></td>
                   <td>
                        <center>
            ...
            <tfoot>
                <tr>
                    ...
                    <!--input type="number" id="cbm_tot_tmp1" name="cbm_tot_tmp1" value="<?php echo $cbm_tot;?>">
...
<script type="text/javascript">
    ...
    var uri5 = '<?php echo $this->uri->segment(5)?>';
    $(document).ready(function() {
        var total = parseFloat($('#totalpr_view').val());
        $('#totalpr').val(total);
        $("#idsupplier,#item_brg_pr,#idcabang").select2({
            placeholder: "Pilih",
            allowClear: true
        });
        ...
        var dataTableItem = $('#prdetailitem').DataTable({
            "sDom": 'Bfrtip',
            "responsive": true,
            "bInfo":false,
            "bPaginate":false,
        ...
        /*
        $('#prdetailitem tbody').on( 'click', 'td', function () {
            var dt = dataTableItem.cell(this).data();
            alert(dt);
        });

        $('#prdetailitem tbody').on( 'click', 'td', function () {
            var cell = dataTableItem.cell(this);
            cell.data( cell.data() + 1 ).draw();
        });
        */
        //document.getElementById("cbm_tot_tmp1").style.visibility = "hidden";
        //document.getElementById("cbm_tot_tmp2").style.visibility = "hidden";
    });
    function get_total(){
      ...
      //var cbm_tot_awal = document.getElementById("cbm_tot_tmp1").value;
      ...
      //document.getElementById("cbm_tot_tmp1").value = parseInt(document.getElementById("cbm_tot_tmp1").value) + parseInt(document.getElementById("cbm_tot").value)
      if (document.getElementById("cbm_tot").value > 60 || cbm_tot_akhir > 60) {
        alert('MAKSIMUM CBM ADALAH 60');
    ...
    function setitembarang(){
        ...
        window.location.href = siteurl+'purchaserequest/create/'+uri3+'/'+uri4+'/'+idbarang;
    }
    function getsupplier(){
        var idsup = $('#idsupplier').val();
        window.location.href = siteurl+'purchaserequest/create/'+uri3+'/'+idsup;
        /*
        if(idsup != ''){
           $.ajax({
                type:"GET",
                url:siteurl+"purchaserequest/get_supplier/"+uri3+'/'+uri4,
                data:"idsup="+idsup,
